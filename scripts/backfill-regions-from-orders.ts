...
async function backfillRegionsFromOrders() {
  console.log('Starting backfill of sales by region data from existing orders...')
    ...
    for (const connection of connections) {
      ...
      while (hasMoreOrders) {
        ...
        for (const order of orders) {
          ...
          if (!defaultAddress.city && !customer.city) {
            continue
          }
          ...
          regionData.push({
            ...
            city: defaultAddress.city || customer.city || 'Unknown',
            province: defaultAddress.province || customer.province || 'Unknown',
            province_code: defaultAddress.province_code || customer.province_code || 'Unknown',
            country: defaultAddress.country || customer.country || 'Unknown',
            country_code: defaultAddress.country_code || customer.country_code || 'Unknown',

-- Product Performance Tracking Tables
...
CREATE POLICY "Users can view their own product performance metrics data"
...
CREATE POLICY "Users can insert their own returns data"
  ON public.product_returns
  FOR INSERT
  WITH CHECK (brand_id IN (SELECT id FROM public.brands WHERE user_id = auth.uid()));

CREATE POLICY "Users can insert their own product relationships data"
  ON public.product_relationships
  FOR INSERT
  WITH CHECK (brand_id IN (SELECT id FROM public.brands WHERE user_id = auth.uid()));

CREATE POLICY "Users can insert their own inventory turnover data"
  ON public.inventory_turnover
  FOR INSERT
  WITH CHECK (brand_id IN (SELECT id FROM public.brands WHERE user_id = auth.uid()));

CREATE POLICY "Users can insert their own product reviews data"
  ON public.product_reviews
  FOR INSERT
  WITH CHECK (brand_id IN (SELECT id FROM public.brands WHERE user_id = auth.uid()));

CREATE POLICY "Users can insert their own product performance metrics data"
  ON public.product_performance_metrics
  FOR INSERT
  WITH CHECK (brand_id IN (SELECT id FROM public.brands WHERE user_id = auth.uid()));
...
BEGIN
    ...
    EXECUTE 'ALTER TABLE product_performance_metrics ENABLE ROW LEVEL SECURITY;';
    EXECUTE 'ALTER TABLE product_relationships ENABLE ROW LEVEL SECURITY;';
    EXECUTE 'ALTER TABLE product_reviews ENABLE ROW LEVEL SECURITY;';
    EXECUTE 'ALTER TABLE product_views ENABLE ROW LEVEL SECURITY;';
    EXECUTE 'ALTER TABLE product_returns ENABLE ROW LEVEL SECURITY;';
    EXECUTE 'ALTER TABLE inventory_turnover ENABLE ROW LEVEL SECURITY;';
    
    -- Create policies for authenticated users
    EXECUTE 'CREATE POLICY product_performance_metrics_policy ON product_performance_metrics 
             FOR ALL TO authenticated 
             USING (true);';
             
    EXECUTE 'CREATE POLICY product_relationships_policy ON product_relationships 
             FOR ALL TO authenticated 
             USING (true);';
             
    EXECUTE 'CREATE POLICY product_reviews_policy ON product_reviews 
             FOR ALL TO authenticated 
             USING (true);';
             
    EXECUTE 'CREATE POLICY product_views_policy ON product_views 
             FOR ALL TO authenticated 
             USING (true);';
             
    EXECUTE 'CREATE POLICY product_returns_policy ON product_returns 
             FOR ALL TO authenticated 
             USING (true);';
             
    EXECUTE 'CREATE POLICY inventory_turnover_policy ON inventory_turnover 
             FOR ALL TO authenticated 
             USING (true);';

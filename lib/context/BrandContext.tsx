...
export function BrandProvider({ children }: { children: React.ReactNode }) {
  ...
  // Load selected brand from localStorage ONLY if user has explicitly selected one before
  useEffect(() => {
  ...
  const setSelectedBrandIdWithPersistence = (id: string | null) => {
    ...
    if (typeof window !== 'undefined') {
      if (id) {
        ...
        // Mark that user has explicitly selected a brand
        localStorage.setItem('hasEverSelectedBrand', 'true')
      } else {
        localStorage.removeItem('selectedBrandId')
        // Keep the hasEverSelected flag so they don't lose persistence ability
      }
    }
  }

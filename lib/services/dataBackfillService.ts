...
export class DataBackfillService {
   ...
   */
  static async triggerInitialBackfill(brandId: string, platformType: 'meta' | 'shopify', accessToken: string) {
    ...
    try {
      ...
      if (platformType === 'meta') {
        ...
        await this.backfillShopifyData(brandId, accessToken)
      }
      ...
      console.log(`[DataBackfill] Completed initial backfill for ${platformType} on brand ${brandId}`)
    } catch (error) {
  ...
  /**
   * Backfill Shopify data for the last 90 days
   */
  private static async backfillShopifyData(brandId: string, accessToken: string) {

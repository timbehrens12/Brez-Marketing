...
const ROLES = [
  {
    id: 'viewer',
    name: 'Viewer',
    description: 'View-only access to reports and brand information',
    color: 'bg-gray-500/20 text-gray-300 border-gray-500/40'
  },
  {
    id: 'media_buyer',
    name: 'Media Buyer',
    description: 'Access to campaigns, reports, and marketing tools',
    color: 'bg-gray-500/20 text-gray-300 border-gray-500/40'
  },
  {
    id: 'admin',
    name: 'Admin',
    description: 'Full access to brand management and settings',
    color: 'bg-gray-500/20 text-gray-300 border-gray-500/40'
  }
]
                ...
                <div className="space-y-2">
                  ...
                  <Select value={selectedRole} onValueChange={setSelectedRole}>
                    ...
                    <SelectContent className="bg-[#1a1a1a] border-[#333] text-white">
                      {ROLES.map(role => (
                        <SelectItem key={role.id} value={role.id} className="text-white hover:bg-[#333]">
                          <div className="flex items-center gap-2">
              ...
              <CardContent className="p-4">
                {shareLinks.length > 0 ? (
                  <div className="space-y-4 max-h-[800px] overflow-y-auto">
                    {shareLinks.map(link => {
                      const linkBrands = ownedBrands.filter(brand => link.brand_ids.includes(brand.id))

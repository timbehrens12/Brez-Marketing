  ...
  const generateTextPromptAddition = () => {
    ...
    if (customText.top || customText.bottom) {
      ...
      if (customText.top) {
        ...
        textAddition += `MANDATORY: Place "${customText.top}" text in the EXACT CENTER of the top open space above the product - find the middle point between the very top edge and where the product begins, then center the text PERFECTLY in that middle area. PERFECTLY CENTERED horizontally with equal spacing from left and right edges. The text must be PERFECTLY STRAIGHT and LEVEL (not tilted or wonky). Use large, bold, readable font that is EVENLY SPACED and SYMMETRICALLY POSITIONED. Make the text color ${topColorName.toLowerCase()} (${textColors.top}). Ensure text is PRECISELY ALIGNED and appears professional. `
      }
      if (customText.bottom) {
        const bottomColorName = colorOptions.find(c => c.value === textColors.bottom)?.name || 'white'
        textAddition += `MANDATORY: Place "${customText.bottom}" text in the EXACT CENTER of the bottom open space below the product - find the middle point between where the product ends and the very bottom edge, then center the text PERFECTLY in that middle area. PERFECTLY CENTERED horizontally with equal spacing from left and right edges. The text must be PERFECTLY STRAIGHT and LEVEL (not tilted or wonky). Use large, bold, readable font that is EVENLY SPACED and SYMMETRICALLY POSITIONED. Make the text color ${bottomColorName.toLowerCase()} (${textColors.bottom}). Ensure text is PRECISELY ALIGNED and appears professional. `
      }
      textAddition += 'CRITICAL TEXT ALIGNMENT: ALL TEXT must be PERFECTLY CENTERED both horizontally and vertically within their designated areas. Text must be STRAIGHT, LEVEL, and EVENLY POSITIONED - NO tilting, skewing, or wonky alignment. Use professional typography spacing with EQUAL margins on all sides. MANDATORY SYMMETRICAL SPACING: Ensure the top and bottom text areas have EXACTLY EQUAL HEIGHT and spacing - measure carefully to create perfect symmetry and balance. Strong contrast, drop shadows, and outlines required for readability. NEVER cut off any letters. DO NOT SKIP THE TEXT OVERLAYS - THEY ARE REQUIRED.'
    }
    return textAddition
  }

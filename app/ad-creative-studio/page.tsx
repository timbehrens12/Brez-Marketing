...
const STYLE_OPTIONS: StyleOption[] = [
  ...
  // MULTI-PRODUCT TEMPLATE - For combining multiple items into one creative
  {
    ...
    prompt: 'Create a professional multi-product showcase featuring the primary clothing item along with additional complementary items. Arrange them in an elegant, balanced composition that showcases each piece effectively. Use consistent professional lighting, maintain high image quality for all items, and create a cohesive visual presentation that highlights the entire collection. The layout should be clean, modern, and visually appealing while ensuring each product remains clearly visible and professionally presented.'
  },
...
const STORAGE_LIMIT = 50 // Maximum saved creatives per brand
  ...
  // New multi-product generation function - Simplified approach
  const generateMultiProductCreative = async (images: File[], style: StyleOption, customText: any, finalName: string): Promise<string> => {
    return new Promise(async (resolve, reject) => {
      try {
        console.log('🎨 Starting simplified multi-product creative generation...')

        // Use the first image as the primary image and enhance the prompt to describe multi-product scenario
        const primaryImage = images[0]

        // Create enhanced prompt that describes the multi-product scenario
        const basePrompt = style.prompt || 'Create a professional product image'
        const multiProductPrompt = `${basePrompt}

        IMPORTANT: This is a multi-product request featuring ${images.length} different items. Create a single cohesive image that showcases all ${images.length} products together in an elegant arrangement. The primary product shown should be combined with ${images.length - 1} additional similar products arranged professionally around it.`
        ...
        console.log('🚀 Sending simplified multi-product generation request...')
        ...
        try {
          ...
          if (!response.ok) {
            ...
            console.error('❌ Multi-product generation failed:', response.status, errorText)
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (imageFiles.length === 1) {
      ...
      // Generate collage
      try {
        const collage = await generateCollage(imageFiles, 'grid-2x2')
        setCollageUrl(collage)
        toast.success(`${imageFiles.length} images uploaded! Collage generated.`)
      } catch (error) {
  ...
  const generateImageFromModal = async () => {
    ...
    if (modalStyle.id === 'custom-template') {
      ...
      // For regular templates, use template prompt + custom instructions
      ...
      enhancedPrompt = modalStyle.prompt + textPromptAddition + customInstructionsAddition + genderAddition
    }
    ...
    try {
      if (isMultiMode && uploadedImages.length > 1) {
        ...
        try {
          ...
          setLoadedImages(prev => {
            const updated = { ...prev }
            delete updated[multiProductCreativeId]
            return updated
          })
        }
      }
  ...
  return (
                ...
                {isMultiMode && uploadedImages.length > 0 ? (
                  <>
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <div className="grid grid-cols-2 gap-1 max-w-[180px] max-h-[110px]">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-6">
                     ...
                     <div>
                       ...
                       <div className="grid grid-cols-1 gap-6">
                         <div
                           onClick={() => openStyleModal(STYLE_OPTIONS.find(s => s.id === 'custom-template')!)}
                           className="bg-gradient-to-br from-[#222] via-[#252525] to-[#1e1e1e] border border-[#333] rounded-xl p-6 transition-all duration-300 group hover:border-green-500/50 hover:shadow-2xl hover:shadow-green-500/10 cursor-pointer"
                         >
                           <div className="flex items-center gap-4">
                             <div className="w-16 h-16 bg-gradient-to-br from-[#333] to-[#222] border border-[#444] rounded-xl flex items-center justify-center">
                             ...
                             <div className="flex-1">
                               <h4 className="text-white text-xl font-bold mb-2">Create Custom Template</h4>
                               <p className="text-gray-300 text-sm leading-relaxed">
                                 Build your own completely custom template with full prompt control. Perfect for unique creative visions and specialized requirements.
                               </p>
                             </div>
                             <div className="opacity-0 group-hover:opacity-100 transition-opacity">
                               <svg className="w-6 h-6 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                       ...
                       <div>
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                           {TEMPLATE_BASE_GROUPS.filter(baseId => {
                             ...
                             if (variant.category !== 'clothing') return false
                             
                             // Apply subcategory filtering for clothing
                             if (selectedCategory === 'clothing-backgrounds') {
                               return getClothingSubcategory(baseId) === 'backgrounds'
                             }
                             if (selectedCategory === 'clothing-environments') {
                               return getClothingSubcategory(baseId) === 'environments'  
                             }
                             if (selectedCategory === 'clothing-models') {
                               return getClothingSubcategory(baseId) === 'models'
                             }
                             
                             // For 'all' and 'clothing' categories, show all clothing templates
                             return selectedCategory === 'all' || selectedCategory === 'clothing'
                           }).map((baseId) => {
                          ...
                          >
                            ...
                            >
                              <div className={`aspect-[3/4] flex items-center justify-center overflow-hidden ${
                                currentVariant.id === 'custom-template' 
                                  ? 'bg-gradient-to-br from-[#2a2a2a] to-[#1e1e1e] border-2 border-green-500/30' 
                                ...
                                <img
                                  ...
                                  className={`w-full h-full object-cover transition-all duration-300 ${
                                    currentVariant.id === 'custom-template'
                                      ? 'opacity-90 group-hover:opacity-100 group-hover:scale-102'
                          ...
                          {creative.status === 'generating' ? (
                            <div className="flex flex-col items-center gap-4 py-12">
                              ...
                              <div className="text-center">
                                ...
                                <p className="text-gray-400 text-xs mb-2">Generating your ad creative with AI...</p>
                            ...
                            (loadedImages[creative.id]?.generated || creative.generated_image_url) ? (
                              <img
                                src={loadedImages[creative.id]?.generated || creative.generated_image_url}
                              ...
                              {creative.status === 'completed' && (
                                <>
                                  <Button
                                    size="sm"
                                    onClick={() => {
                                      const imageUrl = loadedImages[creative.id]?.generated || creative.generated_image_url
                                      if (imageUrl) {
                                ...
                                <Button
                                  ...
                                  variant="outline"
                                  onClick={() => {
                                    // Pre-populate with existing text overlays if they exist
                                    const existingOverlays = creative.text_overlays || { top: '', bottom: '' }
                                    
                                    // Set existing text values
                                    setOverlayCustomText({ 
                                      top: existingOverlays.top || '', 
                                      bottom: existingOverlays.bottom || '' 
                                    })
                                    
                                    // Set presets to 'custom' if text exists, otherwise reset
                                    setOverlaySelectedTopPreset(existingOverlays.top ? 'custom' : '')
                                    setOverlaySelectedBottomPreset(existingOverlays.bottom ? 'custom' : '')
                                    
                                    // Set custom values for the input fields
                                    setOverlayCustomValues({ 
                                      topValue: existingOverlays.top || '', 
                                      bottomValue: existingOverlays.bottom || '' 
                                    })
                                    
                                    // Reset colors to default (could be enhanced to remember colors)
                                    setOverlayTextColors({ top: '#FFFFFF', bottom: '#FFFFFF' })
                                    
                                    // Set the current creative for text editing
                                    setEditingCreative(creative)
                                    setShowTextOverlayModal(true)
                                  }}
                                  className="bg-white/10 border-white/30 text-white hover:bg-white/20 px-2 py-1"
                                  title="Add or edit text overlay"
                                >
    ...
    {/* Completely Redesigned Modal */}
      ...
      <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[99999] p-4" style={{ top: 0, left: 0, right: 0, bottom: 0 }}>
        <div className="bg-gradient-to-br from-[#1a1a1a] via-[#1f1f1f] to-[#161616] rounded-2xl border border-[#333] max-w-5xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
          ...
          <div className="p-6 space-y-6">
                ...
                {/* Custom Template Prompt - only show for custom template */}
                {modalStyle.id === 'custom-template' && (
                  <div className="bg-gradient-to-br from-[#2a2a2a] to-[#1e1e1e] border border-[#333] rounded-lg p-4">
                    <h4 className="text-white font-medium mb-3 text-sm flex items-center gap-2">
                      <span className="text-green-400">✨</span>
                      Complete Custom Template Prompt (Required)
                    </h4>
                    <textarea
                      placeholder="Write your complete custom prompt here. Describe exactly how you want your product displayed, including background, lighting, positioning, style, mood, etc. For example: 'Display this exact product floating in a minimalist studio with soft white lighting, casting subtle shadows on a clean marble surface. The background should be gradient from light gray to white, with professional photography lighting that highlights the product details...'"
                      value={customTemplatePrompt}
                      onChange={(e) => setCustomTemplatePrompt(e.target.value)}
                      rows={6}
                      className="w-full bg-[#333] border border-[#444] rounded-lg px-3 py-2 text-white text-sm placeholder-gray-400 focus:border-green-500/50 focus:outline-none resize-none"
                    />
                    <p className="text-gray-400 text-xs mt-2">
                      💡 <strong>Tip:</strong> Be specific about background, lighting, positioning, and mood. The more detailed, the better your results!
                    </p>
                  </div>
                )}
                ...
                {/* Gender Selection - only show for model-based templates */}
                {modalStyle.category === 'clothing' && modalStyle.id.startsWith('model_') && (
                  <div className="bg-gradient-to-br from-[#222] to-[#1e1e1e] border border-[#333] rounded-lg p-4">
                    ...
                    <p className="text-gray-500 text-xs mt-2">
                      Choose your preferred model gender for this template. This helps generate more targeted results.
                    </p>
                  </div>
                )}
                ...
                {/* Custom Instructions - only show for non-custom templates */}
                {modalStyle.id !== 'custom-template' && (
                  <div className="bg-gradient-to-br from-[#222] to-[#1e1e1e] border border-[#333] rounded-lg p-4">
                    <h4 className="text-white font-medium mb-3 text-sm">Custom Instructions (Optional)</h4>
                    <textarea
                      placeholder="Add specific customizations to this template (e.g., 'make the lighting warmer', 'add more shadow depth', 'position the item slightly to the left')..."
                      value={customInstructions}
                      onChange={(e) => setCustomInstructions(e.target.value)}
                      rows={3}
                      className="w-full bg-[#333] border border-[#444] rounded-lg px-3 py-2 text-white text-sm placeholder-gray-500 focus:border-gray-400 focus:outline-none resize-none"
                    />
                    <p className="text-gray-500 text-xs mt-2">
                      These instructions will be added to the template's base prompt to customize the generation.
                    </p>
                  </div>
                )}
            ...
            >
              ...
              <div className="p-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {retryImage && (
                      <div className="flex items-center gap-3">
                        ...
                        {/* Image Preview */}
                        <div className="w-12 h-12 bg-[#222] border border-[#444] rounded-lg overflow-hidden">
                          <img 
                            src={URL.createObjectURL(retryImage)} 
                            alt="Preview"
                            className="w-full h-full object-cover"
                          />
                        </div>
                ...
                <div className="space-y-3 max-h-[350px] overflow-y-auto pr-2">
                  ...
                  {/* Custom Issue Input */}
                  <div className="mt-4 p-4 bg-gradient-to-br from-[#252525] to-[#1e1e1e] border border-[#444] rounded-lg">
                    <h4 className="text-white font-medium text-sm mb-3 flex items-center gap-2">
                      <Plus className="w-4 h-4" />
                      Custom Issue
                    </h4>
                    <textarea
                      placeholder="Describe the specific issue you want to fix (e.g., 'nike' appears as 'nice' on the tag - fix fine details)"
                      className="w-full bg-[#333] border border-[#444] rounded-lg px-3 py-3 text-white text-sm placeholder-gray-500 focus:border-orange-400/50 focus:outline-none resize-none"
                      rows={3}
                      value={customIssueText}
                      onChange={(e) => setCustomIssueText(e.target.value)}
                    />
                    <p className="text-gray-500 text-xs mt-2">
                      Small text on bottom should say "nike" not "nice" to demonstrate fixing fine details specifically
                    </p>
                    <Button
                      onClick={() => {
                        if (!retryImage) {
                        ...
                        if (!customIssueText.trim()) {
                          toast.error('Please describe the issue you want to fix')
                          return
                        }
                        retryCreativeWithCustomIssue(customIssueText)
                      }}
                      disabled={!retryImage || !customIssueText.trim()}
                      className="w-full mt-3 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Retry with Custom Issue
                    </Button>
                  </div>
                </div>
      ...
      {/* Text Overlay Modal */}
      {showTextOverlayModal && editingCreative && (
        <div 
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) {
              setShowTextOverlayModal(false)
              setEditingCreative(null)
              // Reset overlay state when closing
              setOverlaySelectedTopPreset('')
              setOverlaySelectedBottomPreset('')
              setOverlayCustomValues({ topValue: '', bottomValue: '' })
              setOverlayTextColors({ top: '#FFFFFF', bottom: '#FFFFFF' })
        ...
        >
          <div 
            className="bg-gradient-to-br from-[#1a1a1a] via-[#1f1f1f] to-[#161616] rounded-2xl border border-[#333] max-w-2xl w-full shadow-2xl max-h-[90vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-white">
                    {editingCreative?.text_overlays?.top || editingCreative?.text_overlays?.bottom ? 'Edit Text Overlay' : 'Add Text Overlay'}
                  </h2>
                  <p className="text-gray-400 text-xs">
                    {editingCreative?.text_overlays?.top || editingCreative?.text_overlays?.bottom 
                      ? 'Modify or remove existing text overlays' 
                      : 'Add text overlays to your creative'
                    }
                  </p>
                </div>
              </div>
            ...
            <div className="p-6">
              {/* Image Preview */}
              <div className="mb-6 p-4 bg-gradient-to-br from-[#252525] to-[#1e1e1e] border border-[#333] rounded-lg">
                <h3 className="text-white font-medium text-sm mb-3">Current Creative</h3>
                <div className="w-32 h-32 mx-auto bg-[#222] border border-[#444] rounded-lg overflow-hidden">
                  {(loadedImages[editingCreative.id]?.generated || editingCreative.generated_image_url) && (
                    <img 
                      src={loadedImages[editingCreative.id]?.generated || editingCreative.generated_image_url} 
              ...
              {/* Text Options - Complete text overlay UI matching the generation page */}
              <div className="space-y-6">
                {/* Top Text */}
                <div>
                  <Label className="text-gray-300 text-sm font-medium mb-3 block">Top Text</Label>
                  <div className="grid grid-cols-2 gap-2 mb-3">
                    {Object.entries(textPresets).slice(0, Math.ceil(Object.keys(textPresets).length / 2)).map(([key, preset]) => (
                      <button
                        key={key}
                        onClick={() => {
                          setOverlaySelectedTopPreset(key)
                          if (key === 'none') {
                            setOverlayCustomValues(prev => ({ ...prev, topValue: '' }))
                            setOverlayCustomText(prev => ({ ...prev, top: '' }))
                          } else if (preset.customizable && key === 'custom') {
                            setOverlayCustomValues(prev => ({ ...prev, topValue: '' }))
                            setOverlayCustomText(prev => ({ ...prev, top: '' }))
                          } else {
                            setOverlayCustomValues(prev => ({ ...prev, topValue: preset.value }))
                            setOverlayCustomText(prev => ({ ...prev, top: preset.value }))
                          }
                        }}
                        className={`p-2 text-xs rounded border transition-colors ${
                          overlaySelectedTopPreset === key
                            ? 'bg-white/10 border-white/30 text-white'
                            : 'bg-[#333] border-[#444] text-gray-300 hover:border-[#555]'
                        }`}
                      >
                        {preset.label}
                      </button>
                  ...
                  {overlaySelectedTopPreset && textPresets[overlaySelectedTopPreset as keyof typeof textPresets]?.customizable && (
                    <input
                      type="text"
                      placeholder="Custom value"
                      value={overlayCustomValues.topValue}
                      onChange={(e) => handleOverlayCustomValueChange('top', e.target.value)}
                      className="w-full bg-[#333] border border-[#444] rounded px-3 py-2 text-white text-sm placeholder-gray-500 focus:border-blue-400/50 focus:outline-none"
                    />
                  )}
                  
                  {overlayCustomText.top && (
                    <div className="mt-3">
                      <label className="text-gray-300 text-xs font-medium block mb-2">Text Color</label>
                      <div className="flex gap-2">
                        {colorOptions.map((color) => (
                          <button
                            key={color.value}
                            onClick={() => setOverlayTextColors(prev => ({ ...prev, top: color.value }))}
                            className={`w-6 h-6 rounded border-2 ${
                              overlayTextColors.top === color.value ? 'border-white' : 'border-gray-600'
                            }`}
                            style={{ backgroundColor: color.value }}
                            title={color.name}
                          />
                  ...
                  <div className="bg-[#333] rounded px-3 py-2 mt-3">
                    <p className="text-sm text-gray-300">
                      Preview: <span style={{ color: overlayTextColors.top }}>{overlayCustomText.top || 'None'}</span>
                    </p>
                  </div>
                </div>
                ...
                {/* Bottom Text */}
                <div>
                  <Label className="text-gray-300 text-sm font-medium mb-3 block">Bottom Text</Label>
                  <div className="grid grid-cols-2 gap-2 mb-3">
                    {/* Always include None option first */}
                    <button
                      onClick={() => {
                        setOverlaySelectedBottomPreset('none')
                        setOverlayCustomValues(prev => ({ ...prev, bottomValue: '' }))
                        setOverlayCustomText(prev => ({ ...prev, bottom: '' }))
                      }}
                      className={`p-2 text-xs rounded border transition-colors ${
                        overlaySelectedBottomPreset === 'none'
                          ? 'bg-white/10 border-white/30 text-white'
                          : 'bg-[#333] border-[#444] text-gray-300 hover:border-[#555]'
                      }`}
                    >
                    ...
                    {/* Then show other presets excluding none */}
                    {Object.entries(textPresets).filter(([key]) => key !== 'none').slice(Math.ceil((Object.keys(textPresets).length - 1) / 2)).map(([key, preset]) => (
                      <button
                        key={key}
                        onClick={() => {
                          setOverlaySelectedBottomPreset(key)
                          if (preset.customizable && key === 'custom') {
                            setOverlayCustomValues(prev => ({ ...prev, bottomValue: '' }))
                            setOverlayCustomText(prev => ({ ...prev, bottom: '' }))
                          } else {
                            setOverlayCustomValues(prev => ({ ...prev, bottomValue: preset.value }))
                            setOverlayCustomText(prev => ({ ...prev, bottom: preset.value }))
                          }
                        }}
                        className={`p-2 text-xs rounded border transition-colors ${
                          overlaySelectedBottomPreset === key
                            ? 'bg-white/10 border-white/30 text-white'
                            : 'bg-[#333] border-[#444] text-gray-300 hover:border-[#555]'
                        }`}
                      >
                  ...
                  {overlaySelectedBottomPreset && textPresets[overlaySelectedBottomPreset as keyof typeof textPresets]?.customizable && (
                    <input
                      type="text"
                      placeholder="Custom value"
                      value={overlayCustomValues.bottomValue}
                      onChange={(e) => handleOverlayCustomValueChange('bottom', e.target.value)}
                      className="w-full bg-[#333] border border-[#444] rounded px-3 py-2 text-white text-sm placeholder-gray-500 focus:border-blue-400/50 focus:outline-none"
                    />
                  )}
                  
                  {overlayCustomText.bottom && (
                    <div className="mt-3">
                      <label className="text-gray-300 text-xs font-medium block mb-2">Text Color</label>
                      <div className="flex gap-2">
                        {colorOptions.map((color) => (
                          <button
                            key={color.value}
                            onClick={() => setOverlayTextColors(prev => ({ ...prev, bottom: color.value }))}
                            className={`w-6 h-6 rounded border-2 ${
                              overlayTextColors.bottom === color.value ? 'border-white' : 'border-gray-600'
                            }`}
                            style={{ backgroundColor: color.value }}
                            title={color.name}
                          />
                  ...
                  <div className="bg-[#333] rounded px-3 py-2 mt-3">
                    <p className="text-sm text-gray-300">
                      Preview: <span style={{ color: overlayTextColors.bottom }}>{overlayCustomText.bottom || 'None'}</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="flex gap-3 mt-5 justify-center">
                  <Button
                    ...
                    variant="outline"
                    onClick={() => {
                      setOverlaySelectedTopPreset('none')
                      setOverlayCustomValues(prev => ({ ...prev, topValue: '' }))
                      setOverlayCustomText(prev => ({ ...prev, top: '' }))
                    }}
                ...
                {/* Generate Button */}
                <Button
                  onClick={handleTextOverlayGeneration}
                  className="w-full bg-gradient-to-r from-white/20 to-white/10 hover:from-white/30 hover:to-white/20 text-white border border-white/20"
                >
                  {overlayCustomText.top || overlayCustomText.bottom ? 'Generate with New Text' : 'Generate with No Text'}
                </Button>
              </div>
            </div>
          </div>

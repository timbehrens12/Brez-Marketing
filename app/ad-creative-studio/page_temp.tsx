...
DO NOT ask for more images - I am providing all ${images.length} images now. Generate the combined fashion display image immediately.`
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (imageFiles.length === 1) {
      ...
      // Progress to creative type selection
  ...
  const renderUploadStep = () => (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-gradient-to-br from-[#1a1a1a] via-[#1f1f1f] to-[#161616] rounded-xl border border-[#333] p-8">
        <div className="border-2 border-dashed border-[#333] rounded-xl p-12 text-center hover:border-[#555] transition-colors">
          <div className="flex flex-col items-center gap-4">
            <div className="w-20 h-20 bg-gradient-to-br from-white/5 to-white/10 rounded-xl flex items-center justify-center border border-white/10">
              <Upload className="w-10 h-10 text-gray-400" />
            </div>
            <div>
              <h3 className="text-xl font-semibold text-white mb-2">Drop images here or click to upload</h3>
              <p className="text-gray-400 text-sm">Supports JPG, PNG, WebP • Single or multiple images</p>
            </div>
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-white flex items-center gap-2">
            <div className="w-6 h-6 bg-gradient-to-br from-white/10 to-white/5 rounded flex items-center justify-center text-xs font-bold">T</div>
            Text Overlays
          </h3>
          ...
          <div className="bg-[#2a2a2a] rounded-lg p-4 space-y-3">
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 bg-blue-500/20 rounded flex items-center justify-center border border-blue-500/30">
                <span className="text-xs text-blue-400">↑</span>
              </div>
              <label className="text-white font-medium text-sm">Top Text</label>
            </div>
              ...
              <button
                ...
                className={`p-2 text-xs rounded border transition-all ${
                  selectedTopPreset === 'none'
                    ...
                    : 'bg-[#333] border-[#444] text-gray-300 hover:border-[#555] hover:bg-[#3a3a3a]'
                }`}
              >
                None
              </button>
              {Object.entries(textPresets).filter(([key]) => key !== 'none').slice(0, Math.ceil((Object.keys(textPresets).length - 1) / 2)).map(([key, preset]) => (
                <button
                  key={key}
                  onClick={() => {
                    setSelectedTopPreset(key)
                    if (preset.customizable && key === 'custom') {
                      setCustomValues(prev => ({ ...prev, topValue: '' }))
                      setCustomText(prev => ({ ...prev, top: '' }))
                    } else {
                      setCustomValues(prev => ({ ...prev, topValue: preset.value }))
                      setCustomText(prev => ({ ...prev, top: preset.value }))
                    }
                  }}
                  className={`p-2 text-xs rounded border transition-all ${
            ...
            {(selectedTopPreset === 'custom' || selectedTopPreset === 'percentage' || selectedTopPreset === 'money') && (
              <input
                type="text"
                ...
                placeholder={selectedTopPreset === 'percentage' ? 'e.g., 30% OFF' : selectedTopPreset === 'money' ? 'e.g., $15 OFF' : 'Enter custom text'}
                className="w-full bg-[#444] border border-[#555] rounded px-3 py-2 text-white text-sm placeholder-gray-400 focus:border-[#666] focus:outline-none"
              />
            )}
            ...
            <div className="bg-[#333] rounded px-3 py-2">
              <p className="text-xs text-gray-300">
                Preview: <span style={{ color: textColors.top, fontWeight: 'bold' }}>{customText.top || 'None'}</span>
              </p>
            </div>
          </div>
          ...
          <div className="bg-[#2a2a2a] rounded-lg p-4 space-y-3">
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 bg-purple-500/20 rounded flex items-center justify-center border border-purple-500/30">
                <span className="text-xs text-purple-400">↓</span>
              </div>
              <label className="text-white font-medium text-sm">Bottom Text</label>
            </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              ...
              {Object.entries(textPresets).filter(([key]) => key !== 'none').slice(Math.ceil((Object.keys(textPresets).length - 1) / 2)).map(([key, preset]) => (
                <button
                  key={key}
                  onClick={() => {
                    setSelectedBottomPreset(key)
                    if (preset.customizable && key === 'custom') {
                      setCustomValues(prev => ({ ...prev, bottomValue: '' }))
                      setCustomText(prev => ({ ...prev, bottom: '' }))
                    } else {
                      setCustomValues(prev => ({ ...prev, bottomValue: preset.value }))
                      setCustomText(prev => ({ ...prev, bottom: preset.value }))
                    }
                  }}
                  className={`p-2 text-xs rounded border transition-all ${
            ...
            {(selectedBottomPreset === 'custom' || selectedBottomPreset === 'percentage' || selectedBottomPreset === 'money') && (
              <input
                type="text"
                ...
                placeholder={selectedBottomPreset === 'percentage' ? 'e.g., 30% OFF' : selectedBottomPreset === 'money' ? 'e.g., $15 OFF' : 'Enter custom text'}
                className="w-full bg-[#444] border border-[#555] rounded px-3 py-2 text-white text-sm placeholder-gray-400 focus:border-[#666] focus:outline-none"
              />
            )}
            ...
            <div className="bg-[#333] rounded px-3 py-2">
              <p className="text-xs text-gray-300">
                Preview: <span style={{ color: textColors.bottom, fontWeight: 'bold' }}>{customText.bottom || 'None'}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          <h3 className="text-lg font-semibold text-white flex items-center gap-2">
            <div className="w-6 h-6 bg-gradient-to-br from-white/10 to-white/5 rounded flex items-center justify-center text-xs font-bold">+</div>
            Additional Instructions
          </h3>
          <div className="bg-[#2a2a2a] rounded-lg p-4">
            <textarea
              ...
              placeholder="Any specific requirements or styling preferences..."

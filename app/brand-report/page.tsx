    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] via-[#111111] to-[#0f0f0f] flex flex-col">
      <div className="flex-1 p-4 pb-6">
        ...
        {isLoadingReport ? (
          ...
          (() => {
            // Parse the report content to extract just the content and footer
            const parser = new DOMParser();
            const htmlDoc = parser.parseFromString(selectedReport.content, 'text/html');
            
            // Extract content and footer sections
            const reportContent = htmlDoc.querySelector('.report-content');
            const reportFooter = htmlDoc.querySelector('.report-footer');
            const reportStyles = htmlDoc.querySelector('style');
            
            // Get brand info from the header if available
            const brandName = htmlDoc.querySelector('.brand-name')?.textContent || brands.find(b => b.id === selectedBrandId)?.name || 'Unknown Brand';
            const reportPeriod = htmlDoc.querySelector('.info-value')?.textContent || selectedPeriod;
            const reportId = htmlDoc.querySelector('.report-id')?.textContent || 'N/A';
            
            return (
              <div className="space-y-0">
                {/* Inject the styles */}
                <style dangerouslySetInnerHTML={{ __html: reportStyles?.innerHTML || '' }} />
                
                {/* Custom Header for Page View */}
                <div className="bg-gradient-to-br from-[#1a1a1a] via-[#1f1f1f] to-[#161616] rounded-xl border border-[#333] shadow-xl overflow-hidden">
                  <div className="bg-gradient-to-r from-[#1a1a1a] to-[#2a2a2a] p-6 border-b border-[#333]">
                    <div className="flex items-center justify-between">
                      <div>
                        <h1 className="text-3xl font-bold text-white mb-2">MARKETING PERFORMANCE REPORT</h1>
                        <p className="text-gray-300">Comprehensive marketing analysis and performance insights for {brandName}</p>
                      </div>
                      <div className="bg-[#2a2a2a] border border-[#444] rounded-xl p-4 flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-xs text-gray-400 uppercase tracking-wider">Brand</div>
                          <div className="text-white font-bold">{brandName}</div>
                        </div>
                      </div>
                    </div>
                    
                    <div className="bg-[#2a2a2a]/60 border border-[#333] rounded-lg p-3 mt-4 inline-block">
                      <div className="flex items-center gap-6 text-sm">
                        <div className="flex justify-between items-center gap-4">
                          <span className="text-gray-400 uppercase tracking-wider text-xs">Period</span>
                          <span className="text-white font-bold">{reportPeriod}</span>
                        </div>
                        <div className="flex justify-between items-center gap-4">
                          <span className="text-gray-400 uppercase tracking-wider text-xs">Generated</span>
                          <span className="text-white font-bold">{format(new Date(), 'MMM dd, yyyy')}</span>
                        </div>
                        <div className="flex justify-between items-center gap-4">
                          <span className="text-gray-400 uppercase tracking-wider text-xs">Report ID</span>
                          <span className="text-white font-mono text-xs">{reportId.replace('ID: ', '')}</span>
                        </div>
                        <div className="flex justify-between items-center gap-4">
                          <span className="text-gray-400 uppercase tracking-wider text-xs">Generated By</span>
                          <span className="text-white font-bold">{userFirstName || 'Unknown User'}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ 
                      __html: reportContent?.outerHTML || '<div class="p-6 text-white">No content available</div>'
                    }}
                  />

...
export async function POST(request: NextRequest) {
  try {
    ...
    // 🚨 IMMEDIATE SYNC: Run inline to prevent silent failures
    console.log(`[Meta Exchange] Starting immediate inline sync to prevent silent failures`)
    ...
    try {
      ...
      console.error('[Meta Exchange] Critical: Background sync completely failed:', backgroundSyncError)
      console.error('[Meta Exchange] Error details:', {
        message: backgroundSyncError.message,
        stack: backgroundSyncError.stack,
        brandId: state,
        connectionId: connectionData.id
      })
    }

    // 🚨 IMMEDIATE FIX: Mark as completed to prevent 85% stuck UI (moved to end)
    await supabase
      .from('platform_connections')
      .update({
        sync_status: 'completed',
    ...
    console.log(`[Meta Exchange] ✅ Auth exchange completed - sync status marked as completed`)

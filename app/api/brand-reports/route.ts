...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`Loading reports for brand ${brandId}, user ${userId}, period ${periodName}${snapshotTime ? `, snapshot: ${snapshotTime}` : ''}`)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!brandId || !userId || !fromDate || !toDate || !periodName || !reportContent || !snapshotTime) {
      return NextResponse.json(
        { error: 'Missing required parameters (including snapshotTime)' }, 
        { status: 400 }
      )
    }
    ...
    // Check if a report already exists for this exact snapshot time

...
export async function POST(request: NextRequest) {
  try {
    ...
    if (dateFrom && dateTo) {
      ...
      // Default: backfill last 90 days for initial sync
      endDate = subDays(today, 1) // Yesterday
      startDate = subDays(endDate, days - 1) // 90 days back from yesterday
    }
    ...
    // 5. Log this backfill in the history table
    try {
      await supabase
    ...
    // Update sync status to 'failed' on any server error
    try {
      const supabase = createClient(
        process.env.NEXT_PUBLIC_SUPABASE_URL!,
        process.env.SUPABASE_SERVICE_ROLE_KEY!
      ...
      console.error('[Meta Backfill] Failed to update sync status to failed:', updateError)
    }

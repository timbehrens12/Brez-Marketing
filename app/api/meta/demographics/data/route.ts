...
export async function GET(request: NextRequest) {
  try {
    ...
    // If no data found for the requested date range, return empty (don't use fallback)
    // This ensures we show accurate empty states when there's no spend for the selected dates
    if (!data || data.length === 0) {
      console.log(`[Demographics API] No data found for ${finalDateFrom} to ${finalDateTo} - returning empty array (no fallback)`)
      data = []
    }

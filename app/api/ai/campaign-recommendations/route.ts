...
export async function PATCH(request: NextRequest) {
  try {
    ...
    // Update the recommendation status in the metadata
    const updatedMetadata = {
      ...existingLogs.metadata,
      recommendation: {
        ...existingLogs.metadata.recommendation,
        status: status
      }
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { brandId, campaignId, campaignData, forceRefresh = false } = await request.json()
    ...
    // Check for existing valid recommendation unless force refresh
    if (!forceRefresh) {
      ...
      if (existingRecommendation && !fetchError) {
        console.log('[AI Campaign Recommendations] Found valid cached recommendation')
        return NextResponse.json({
    ...
    console.log('[Campaign Recommendations] Next refresh calculation:', {
    ...
    // Save recommendation to cache table
...
`
  ...
  const prompt = `
You are an expert Meta advertising strategist. Analyze the following campaign data including detailed adset and ad performance with 7-day historical trends, and CRITICALLY evaluate the effectiveness of any previous recommendations. Provide a specific, actionable recommendation that considers granular performance at the adset and ad level.

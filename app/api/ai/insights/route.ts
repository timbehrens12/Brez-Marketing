  ...
  try {
    ...
    if (metaConnection) {
      ...
      if (!metaError && metaData && metaData.length > 0) {
        ...
        const totalSpend = metaData.reduce((sum: number, item: any) => sum + (item.spend || 0), 0);
        const totalImpressions = metaData.reduce((sum: number, item: any) => sum + (item.impressions || 0), 0);
        const totalClicks = metaData.reduce((sum: number, item: any) => sum + (item.clicks || 0), 0);

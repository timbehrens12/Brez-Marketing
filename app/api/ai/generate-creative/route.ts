...
const WEEKLY_CREATIVE_LIMIT = 10; // 10 creative generations per week
...
${backgroundPreset.prompt}`;
    ...
    if (error.code === 'invalid_image') {
      return NextResponse.json(
        { error: 'Invalid image format. Please use PNG or JPEG format.' },
        { status: 400 }
      );
    }
    ...
    if (error.code === 'content_policy_violation') {
      return NextResponse.json(
        { error: 'Image content violates OpenAI policy. Please use appropriate product images.' },
        { status: 400 }
      );
    }
    ...
    if (error.message?.includes('vision')) {
      return NextResponse.json(
        { error: 'Failed to analyze image. Please try with a clearer product photo.' },
        { status: 400 }
      );
    }

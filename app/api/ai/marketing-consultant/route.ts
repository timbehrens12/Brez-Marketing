...
// Agency mode usage tracking using ai_feature_usage table
async function checkAgencyModeUsage(userId: string, featureType: string, supabase: any) {
  try {
    const today = getCurrentLocalDateString()
    const dailyLimit = 15 // Same as brand mode
    
    // Get today's usage count from ai_feature_usage table
    ...
    if (error) {
      console.error('Error checking agency mode usage:', error)
      return { canUse: false, reason: 'Database error' }
    }
    
    const todayUsageCount = usageData?.length || 0
    ...
    console.error('Error in checkAgencyModeUsage:', error)
...
async function recordAgencyModeUsage(userId: string, featureType: string, metadata: any, supabase: any) {
  try {
    ...
    if (error) {
      console.error('Error recording agency mode usage:', error)
      return false
    }
    
    return true
  } catch (error) {
    console.error('Error in recordAgencyModeUsage:', error)
    return false
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (mode === 'agency') {
      // For agency mode, use user-based tracking instead of brand-based
      usageStatus = await checkAgencyModeUsage(userId, 'ai_consultant_chat', supabase)
    } else {
      // For brand mode, use existing brand-based tracking
      usageStatus = await aiUsageService.checkUsageStatus(
        brandId,
        userId, 
        'ai_consultant_chat'
      )
    ...
    if (mode === 'agency') {
      // For agency mode, use user-based tracking
    ...
    if (mode === 'agency') {
      updatedStatus = await checkAgencyModeUsage(userId, 'ai_consultant_chat', supabase)
    } else {
      updatedStatus = await aiUsageService.checkUsageStatus(
        brandId, 
        userId, 
        'ai_consultant_chat'
      )
...
async function gatherComprehensiveMarketingData(supabase: any, brandId: string, customDateRange?: { from?: string, to?: string, days?: number }) {
  ...
  try {
    ...
    const shopifyData = await gatherShopifyData(supabase, brandId, fromDate, customDateRange?.to || todayStr)

    // Fetch Meta demographic and device data for audience insights
    const metaInsightsData = await gatherMetaInsights(supabase, brandId, fromDate, customDateRange?.to || todayStr)
...
async function gatherShopifyData(supabase: any, brandId: string, fromDate: string, toDate: string) {
  try {
    ...
    if (!connection) {
      console.log('No Shopify connection found for brand')
      return { 
        customers: [], 
        orders: [], 
        products: [], 
    ...
    // Fetch enhanced orders data with comprehensive fields
  ...
  try {
    ...
    const insights = {
      topAgeGroups: demographics.age
        ...
        .slice(0, 3)
        .map(item => ({ age: item.breakdown_value, impressions: item.impressions, spend: item.spend })),
      
      genderDistribution: demographics.gender
        .map(item => ({ gender: item.breakdown_value, impressions: item.impressions, spend: item.spend, ctr: item.ctr })),
      
      topDevices: devicePerformance.device
        .sort((a, b) => b.impressions - a.impressions)
        .slice(0, 3)
        .map(item => ({ device: item.breakdown_value, impressions: item.impressions, ctr: item.ctr })),
      
      bestPlacements: devicePerformance.placement
        .sort((a, b) => b.ctr - a.ctr)
        .slice(0, 3)
        .map(item => ({ placement: item.breakdown_value, ctr: item.ctr, spend: item.spend })),
      
      platformBreakdown: devicePerformance.platform
        .map(item => ({ platform: item.breakdown_value, impressions: item.impressions, spend: item.spend, ctr: item.ctr }))
    }
...
DATA ACCESS: You have access to ${analysisData.dateRange?.days || 90} days of campaign data (${analysisData.dateRange?.from || 'N/A'} to ${analysisData.dateRange?.to || 'N/A'}). You can analyze ANY time period the user requests naturally. If they ask about spending, performance, or metrics WITHOUT specifying a timeframe (like "how much have I spent on ads?"), ask them to clarify the time period they want to analyze (e.g., "For what time period? Last month, last quarter, this year?"). If they mention a specific time period, I'll automatically pull that data.
...
- Analysis Period: ${dateRange?.days || 90} days (${dateRange?.from || 'N/A'} to ${dateRange?.to || 'N/A'})
...
- Returning Customers: ${analysisData.shopifyData.metrics?.returningCustomers || 0}
- High-Value Customers (2x AOV): ${analysisData.shopifyData.metrics?.highValueCustomers || 0}
- Customer Conversion Rate: ${(analysisData.shopifyData.conversionFunnel?.customerConversionRate || 0).toFixed(1)}%
- Repeat Purchase Rate: ${(analysisData.shopifyData.conversionFunnel?.repeatPurchaseRate || 0).toFixed(1)}%
...
async function gatherAgencyWideData(supabase: any, userId: string, customDateRange?: { from?: string, to?: string, days?: number }) {
  try {
    ...
    for (const brand of brands) {
      try {
        ...
        console.log(`[AI Marketing Consultant] Brand ${brand.name} data:`, {
          ...
          totalRevenue: brandData.analysis.totalRevenue,
    ...
    console.error('[AI Marketing Consultant] Stack trace:', error.stack)
    return { 
      campaigns: [], 
      brands: [],
      analysis: { totalSpend: 0, averageROAS: 0 },

        ...
        if (checkError) {
          ...
          if (checkError.code === 'PGRST116') {
            ...
            try {
              ...
              // Trigger comprehensive data backfill
              console.log(`[Shopify Callback] Triggering comprehensive data backfill for brand ${brandId}`)
              DataBackfillService.triggerInitialBackfill(brandId, 'shopify', tokenData.access_token)
                .catch(error => {
                  console.error('[Shopify Callback] Background backfill failed:', error)
                })

              // Set success redirect
              redirectUrl = `/settings?success=true&connectionId=${connection.id}&backfill=started`;

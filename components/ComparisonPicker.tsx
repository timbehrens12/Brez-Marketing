...
export function ComparisonPicker({ comparisonType, customRange, onComparisonChange }: ComparisonPickerProps) {
  const [open, setOpen] = React.useState(false)
  const [calendarOpen, setCalendarOpen] = React.useState(false)
  const [dateRange, setDateRange] = React.useState<DateRange | undefined>(customRange)
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button 
            ...
            role="combobox" 
            aria-expanded={open} 
            className="w-[180px] justify-between bg-gray-300 border border-black text-black hover:text-white hover:bg-gray-400"
          >

...
export function Sidebar({ className }: SidebarProps) {
  ...
  // useActionCenter already applies muting filter and returns filtered counts
  ...
  const [, setSidebarRenderTrigger] = useState(0)
  useEffect(() => {
    if (actionCenterCounts && lastUpdated) {
      console.log('[Sidebar] 📱 Notification counts updated:', actionCenterCounts.totalItems)
      setSidebarRenderTrigger(prev => prev + 1) // Force re-render
    }

...
export function AgencyActionCenter({ dateRange, onLoadingStateChange }: AgencyActionCenterProps) {
  ...
  const { refreshCounts, forceRefresh, actionCenterCounts, lastUpdated } = useActionCenter()
  
  // Force component re-render when notification counts change (this ensures UI updates)
  const [, setRenderTrigger] = useState(0)
  useEffect(() => {
    if (actionCenterCounts && lastUpdated) {
      console.log('[Agency Center] 📱 Notification counts updated:', actionCenterCounts)
  ...
  const markBrandAsRead = (brandId: string) => {
    ...
    // Trigger notification refresh when brand reports are marked as read
    console.log('[Agency Center] Brand health report marked as read, triggering notification refresh')
    if (forceRefresh) {
      forceRefresh() // Use forceRefresh for immediate updates
    }
  ...
  const markAllBrandsAsRead = () => {
    ...
    // Trigger notification refresh when all brand reports are marked as read
    console.log('[Agency Center] All brand health reports marked as read, triggering notification refresh')
    if (forceRefresh) {
      forceRefresh() // Use forceRefresh for immediate updates
    }
  ...
  const handleToolUsed = useCallback((toolId: string) => {
    ...
    // Trigger notification refresh after tool usage
    console.log('[Agency Center] Tool usage recorded, triggering notification refresh')
    if (forceRefresh) {
      setTimeout(() => forceRefresh(), 100) // Use forceRefresh with shorter delay for immediate updates
    }
  }, [markTaskComplete, userId, forceRefresh])
  ...
  }, [userId, handleToolUsed, forceRefresh, loadUserData, generateTodos, loadBrandHealthData])
                  ...
                  <p className="text-gray-300">
                    {new Date().getHours() < 12 
                      ...
                      : new Date().getHours() < 17 
                        ? "Good afternoon" 
                        : "Good evening"}! Ready to optimize your campaigns?
                  </p>
                </div>
              </div>
            </div>

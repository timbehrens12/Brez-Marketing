...
interface DarkRevenueCalendarProps {
  brandId: string;
...
export function DarkRevenueCalendar({ brandId, isRefreshing = false }: DarkRevenueCalendarProps) {
          ...
          <div className="grid grid-cols-7 gap-1 flex-1">
            ...
            {displayData.map((item, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`flex flex-col rounded-md overflow-hidden min-h-[40px] border border-gray-800 bg-[#131722] ${isToday ? 'ring-1 ring-gray-500' : ''}`}
                >
            ...
            <div className="grid grid-cols-12 gap-2">
              {displayData.slice(0, 12).map((item, index) => {
                ...
                return (
                  <div 
                    key={index}
                    className={`flex flex-col rounded-md overflow-hidden border border-gray-800 bg-[#131722] h-20 ${isCurrentHour ? 'ring-1 ring-gray-500' : ''}`}
                  >
                    <div className="text-center py-1 text-xs font-medium bg-[#1a1f2c] text-gray-300">
                      {item.displayDate}
                    </div>
        ...
        <div className="grid grid-cols-4 gap-2 h-full">
          {displayData.map((item, index) => {
            ...
            return (
              <div 
                key={index}
                className={`flex flex-col rounded-md overflow-hidden border border-gray-800 bg-[#131722] h-full ${isCurrentMonth ? 'ring-1 ring-gray-500' : ''}`}
              >
  ...
  return (
    <Card className="h-full bg-[#131722] border-gray-800">
      <div className="p-4 pb-2">
        <div className="flex justify-between items-center">
          ...
          <div className="flex space-x-1">
            <button
              className={`text-xs h-7 px-3 rounded-md transition-all ${timeFrame === 'today' ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white'}`}
              onClick={() => handleTimeFrameChange('today')}
            >
              Today
            </button>
            <button
              className={`text-xs h-7 px-3 rounded-md transition-all ${timeFrame === 'weekly' ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white'}`}
              onClick={() => handleTimeFrameChange('weekly')}
            >
              Week
            </button>
            <button
              className={`text-xs h-7 px-3 rounded-md transition-all ${timeFrame === 'monthly' ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white'}`}
              onClick={() => handleTimeFrameChange('monthly')}
            >
              Month
            </button>
            <button
              className={`text-xs h-7 px-3 rounded-md transition-all ${timeFrame === 'yearly' ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white'}`}
      ...
      <div className="p-2 border-t border-gray-800 text-sm font-medium flex justify-between items-center">

...
import { ProductPerformanceSyncButton } from './ProductPerformanceSyncButton'
...
export function ProductPerformanceWidget({ brandId, isRefreshing = false }: ProductPerformanceWidgetProps) {
  ...
  useEffect(() => {
    ...
    const fetchConnections = async () => {
      try {
        const response = await fetch(`/api/shopify/connections?brandId=${brandId}`)
    ...
    const fetchProductPerformance = async () => {
      ...
      try {
        ...
        if (data.isMockData) {
          ...
          setSyncMessage('Using demo data. Click "Sync Product Data" to fetch real data from your Shopify store.')
        } else {
          setSyncMessage(null)
        }
    ...
    <Card className="col-span-3">
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          ...
          {connectionId && (
            <ProductPerformanceSyncButton connectionId={connectionId} />
          )}
        </div>
        {syncMessage && (

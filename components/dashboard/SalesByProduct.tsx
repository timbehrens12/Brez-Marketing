...
export function SalesByProduct({ brandId, dateRange, isRefreshing = false }: SalesByProductProps) {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      // Use the exact date range from the date picker or force last 30 days
      let fromDate = dateRange.from;
      let toDate = dateRange.to;
      
      // If this is the Last 30 days preset, ensure we're using the correct range
      if (isLast30Days) {
        const today = new Date();
        toDate = endOfDay(today);
        fromDate = startOfDay(subDays(today, 30));
        console.log('[SalesByProduct] Using exact Last 30 days date range:', 
  ...
  const fetchProductSalesData = useCallback(async () => {
    ...
    try {
      ...
      if (isLast30Days) {
        ...
        toDate = endOfDay(today);
        fromDate = startOfDay(subDays(today, 30));
        console.log('[SalesByProduct] Using exact Last 30 days date range for product data:', 
          format(fromDate, 'yyyy-MM-dd'), 'to', format(toDate, 'yyyy-MM-dd'));
      }

...
export function HomeTab({
  ...
  // Function to fetch campaign data from the API - moved up to fix reference error
    ...
    try {
      ...
      if (dateRange?.from && dateRange?.to) {
        ...
        if (!forceRefresh && 
          ...
          console.log('[HomeTab] Skipping campaign fetch as dates are unchanged');
  ...
  useEffect(() => {
    if (isRefreshingData) {
      ...
      // If we have any Meta widgets and a Meta connection, fetch Meta data
      if (validWidgets.some(widget => widget.type === 'meta') && metaConnection) {
        console.log("[HomeTab] Refreshing Meta data due to parent trigger");
        fetchMetaData();
        // If we have a campaign widget, also fetch campaigns with force refresh
        if (validWidgets.some(widget => widget.id === 'meta-campaigns')) {
          console.log("[HomeTab] Refreshing Meta campaigns due to parent trigger");
          fetchCampaigns(true);
        }
      }
      
      // If we have any Shopify widgets and a Shopify connection, fetch Shopify data
      if (validWidgets.some(widget => widget.type === 'shopify') && shopifyConnection) {
        console.log("[HomeTab] Refreshing Shopify data due to parent trigger");
        fetchShopifyData();
      }
    }
  }, [isRefreshingData, metaConnection, shopifyConnection, validWidgets, fetchMetaData, fetchCampaigns, fetchShopifyData]);

  // Comment out the global event listeners to prevent duplicate refreshes
  /* useEffect(() => {
  ...
  // The fetchCampaigns function has been moved above to fix reference errors

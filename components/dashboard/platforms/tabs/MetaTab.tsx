...
`
  ...
  const fetchCtrDirectly = async () => {
    ...
    try {
      const fromDate = toLocalISODateString(dateRange.from);
      const toDate = toLocalISODateString(dateRange.to);
      ...
      const url = `/api/meta/metric/ctr?brandId=${brandId}&from=${fromDate}&to=${toDate}&prevFrom=${prevFrom}&prevTo=${prevTo}`;
      
      console.log(`[MetaTab] Fetching CTR data from ${url}`);
      
      const response = await fetch(url);
      const data = await response.json();
      
      console.log(`[MetaTab] CTR API response:`, data);
      
      // Ensure we received a valid current value and previous value
      const currentValue = data?.value !== undefined && !isNaN(data.value) ? Number(data.value) : 0;
      const previousValue = data?.previous !== undefined && !isNaN(data.previous) ? Number(data.previous) : 0;
      
      console.log(`[MetaTab] Parsed CTR values - current: ${currentValue}, previous: ${previousValue}`);
      
      // Set the data state with properly formatted values
      setCtrData({
        value: currentValue,
      ...
      console.error("[MetaTab] Error fetching CTR data:", error);
  ...
  const refreshAllMetricsDirectly = useCallback(async () => {
    ...
    // Set global refreshing state first for UI feedback
    setIsManuallyRefreshing(true)
    
    // Set loading states for all widgets in a single React update cycle
  ...
  useEffect(() => {
    if (isRefreshingData && brandId) {
      ...
      // Set our internal refreshing state to sync with parent
      setIsManuallyRefreshing(true);

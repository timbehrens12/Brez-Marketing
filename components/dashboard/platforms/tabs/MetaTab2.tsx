...
export function MetaTab2({
  ...
  const syncMetaInsights = async () => {
    ...
    try {
      ...
      if (!previousResponse.ok) {
        ...
        console.warn(`[MetaTab2] ⚠️ Failed to sync previous period Meta insights: ${errorData.error || 'Unknown error'}`);
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          ...
          console.log('Demographic auto-sync failed:', error);
        }
      } else {
  ...
  // Initial data load and date range change handling
  useEffect(() => {
    if (brandId && dateRange?.from && dateRange?.to && metaConnection) {

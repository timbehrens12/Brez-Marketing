...
export function MetaTab2({
  ...
  useEffect(() => {
    if (brandId && dateRange?.from && dateRange?.to && metaConnection) {
      // Only sync on initial mount, not on every dependency change to prevent triple refresh
      if (!hasFetchedMetaData.current) {
        console.log("[MetaTab2] Triggering initial data sync on mount only");
        syncMetaInsights();
        hasFetchedMetaData.current = true;
      } else {
        console.log("[MetaTab2] Skipping automatic refresh - data already fetched. Use events for updates.");
      }
    } else {

...
const CampaignWidget = ({ 
  ...
  useEffect(() => {
    ...
    const handleForceRefresh = (event: any) => {
      ...
      // Skip if already refreshing or refreshed too recently
      ...
      // Apply throttling to avoid too many forced refreshes
      if (!throttle('campaign-force-refresh', 2000)) {
      ...
      // Call the onRefresh handler passed from parent
      if (typeof onRefresh === 'function') {
        const limitedCampaigns = campaigns.slice(0, 10); // Prevent overwhelming API
        logger.debug(`[CampaignWidget] Limited refresh to first ${limitedCampaigns.length} campaigns`);
        
        try {
          // Call the provided refresh handler and set lastRefresh time
          onRefresh(true);
          lastRefresh.current = now;
        } catch (err) {
          console.error('[CampaignWidget] Error in onRefresh handler:', err);
        } finally {
          // Clear refreshing state after a minimum time to avoid UI flicker
          setTimeout(() => {
            if (isMountedRef.current) {
              setRefreshing(false);
            }
  ...
  useEffect(() => {
    ...
    const handleMetaRefresh = (event: Event) => {
      if (event instanceof CustomEvent && event.detail?.brandId === brandId) {
        console.log(`[CampaignWidget] Received meta refresh event:`, event.detail);
    ...
    const handlePageRefresh = (event: Event) => {
      if (event instanceof CustomEvent && event.detail?.brandId === brandId) {
        console.log(`[CampaignWidget] Received page refresh event:`, event.detail);
        handlePlatformRefresh(event);
      }
    };

    // Add direct handler for metaDataRefreshed with high priority  
    const handleMetaDataRefreshed = (event: Event) => {
      if (event instanceof CustomEvent && event.detail?.brandId === brandId) {
        console.log(`[CampaignWidget] Received metaDataRefreshed event:`, event.detail);
        
        // Force a refresh of campaigns when the refresh button is clicked
        setRefreshing(true);
        
        // Clear the campaign cache to ensure fresh data
        setCampaignsWithAdSets(new Set());
        
        // Refresh the campaigns with force=true to get fresh data
        onRefresh(true);
        
        // Expand the campaign that was expanded (if any) to refresh its ad sets
        if (expandedCampaign) {
          const timeoutId = setTimeout(() => {
            if (isMountedRef.current) {
              fetchAdSets(expandedCampaign, true);
    ...
    // Use direct handler for metaDataRefreshed to ensure it runs immediately
  ...
  }, [brandId, handlePlatformRefresh, localCampaigns.length, onRefresh, expandedCampaign, fetchAdSets]);
  ...
  const getCampaignReach = (campaign: Campaign): number | null => {
    // 1. Use the reach value provided in the campaign prop (from the backend API)
    // Check if it exists and is a valid number
    if (campaign && typeof campaign.reach === 'number' && !isNaN(campaign.reach)) {
        console.log(`Using CAMPAIGN PROP reach for ${campaign.campaign_id}: ${campaign.reach}`);
        // Store the calculated value for this campaign (optional, but keeps structure)
        calculatedReachValues.current[campaign.campaign_id] = campaign.reach;
        return campaign.reach;
    }

    // -- Fallback logic if campaign.reach is missing or invalid --
    let reachValue: number | null = null;
    ...
    if (expandedCampaign === campaign.campaign_id && adSets.length > 0) {
      reachValue = adSets.reduce((sum: number, adSet: AdSet) => sum + (Number(adSet.reach) || 0), 0);
    ...
    // 3. Check proactively fetched data (less accurate sum)
      ...
      if (preloadedAdSets && preloadedAdSets.length > 0) {
        // Calculate total, but ensure it's reasonable
        const totalReach = preloadedAdSets.reduce((sum: number, adSet: AdSet) => sum + (Number(adSet.reach) || 0), 0);

        // If total reach is suspiciously high, don't use it
        if (totalReach > 5000) {
          console.warn(`Suspicious preloaded reach value (${totalReach}) for campaign ${campaign.campaign_id}. Ignoring (fallback).`);
        } else {
          reachValue = totalReach;
          console.log(`Using PRELOADED reach (fallback) for campaign ${campaign.campaign_id}: ${reachValue}`);
        }
      } else {
        // 4. Otherwise, return null (will show placeholder until data is loaded)
        console.log(`No fallback ad set data available for campaign ${campaign.campaign_id}. Returning null for reach.`);
      }
    }
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button 
              ...
              disabled={refreshing || isLoading || isPreloading}
              className="h-7 text-xs bg-transparent border-[#333] text-white hover:bg-black/20"
            >
              {refreshing ? (
                <div className="animate-spin h-3.5 w-3.5 mr-1.5 border-2 border-white/20 border-t-white rounded-full"></div>
              ) : (
            ...
            <Button variant="outline" size="sm" onClick={onSync} disabled={isSyncing} className="text-white border-[#333] hover:bg-black/20">
              {isSyncing ? (
                <div className="flex items-center">
                  <div className="animate-spin h-3.5 w-3.5 mr-2 border-2 border-white/20 border-t-white rounded-full"></div>
                  Syncing...
                </div>
              ) : (
                <>
                      ...
                      <React.Fragment key={campaign.id}>
                        ...
                        >
                          ...
                          {visibleMetrics.map(metricId => {
                            ...
                            if (metricId === 'reach') {
                              value = getCampaignReach(campaign); // Will be number or null

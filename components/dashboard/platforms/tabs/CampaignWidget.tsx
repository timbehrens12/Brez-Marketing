...
export function CampaignWidget({ brandId, campaigns, isLoading, isSyncing, dateRange, onRefresh, onSync }: CampaignWidgetProps) {
  ...
  const checkCampaignStatuses = useCallback((campaignsToCheck: Campaign[], forceRefresh = false) => {
    ...
    // Process more campaigns when forceRefresh is true
    const batchSize = forceRefresh ? 20 : 5;
    ...
    campaignsToProcess.forEach((campaign, index) => {
      ...
      }, index * (forceRefresh ? 100 : 200)); // Reduce delay when force refreshing
    });
    ...
    }, (campaignsToProcess.length * 200) + 1000); // Wait for all status checks plus a buffer
  ...
  useEffect(() => {
    const handleForceRefresh = (event: any) => {
      ...
      // Do a full force check of ALL campaign statuses
      checkCampaignStatuses(campaigns, true);
    ...
    // Handle direct force refresh with higher priority
    const handleDirectForceRefresh = (event: any) => {
      ...
      // Always use the bulk refresh for direct force refresh events - this is more reliable
      bulkRefreshCampaignStatuses();
    };

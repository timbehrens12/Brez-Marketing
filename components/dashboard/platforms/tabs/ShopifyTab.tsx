...
export function ShopifyTab({ 
  ...
  // Listen for global refresh events from GlobalRefreshButton
  useEffect(() => {
    const handleGlobalShopifyRefresh = (event: CustomEvent) => {
      console.log("[ShopifyTab] Received global Shopify refresh event:", event.detail);
      
      // Only process if this is for our brand
      if (event.detail?.brandId !== brandId) {
        console.log("[ShopifyTab] Ignoring global refresh for different brand");
        return;
      }
      
      console.log("[ShopifyTab] Processing global Shopify refresh, triggering force refresh");
      
      // Dispatch a force refresh event with cache busting
      const today = new Date();
      const isToday = dateRange && dateRange.from && dateRange.to && 
    ...
    const handleGlobalWidgetRefresh = (event: CustomEvent) => {
      console.log("[ShopifyTab] Received global widget refresh event:", event.detail);
      
      // Only process if this is for our brand and includes Shopify
      if (event.detail?.brandId !== brandId || !event.detail?.platforms?.shopify) {
        console.log("[ShopifyTab] Ignoring global refresh - not for this brand or no Shopify platform");
        return;
      }
      
      console.log("[ShopifyTab] Processing global widget refresh for Shopify");
      handleGlobalShopifyRefresh(event);
    };

    if (typeof window !== 'undefined') {
      window.addEventListener('force-shopify-refresh', handleGlobalShopifyRefresh as EventListener);
    ...
    return () => {
      if (typeof window !== 'undefined') {
        window.removeEventListener('force-shopify-refresh', handleGlobalShopifyRefresh as EventListener);

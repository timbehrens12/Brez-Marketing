...
export function GlobalRefreshButton({ brandId, activePlatforms, currentTab = 'site', connections = [], dateRange }: GlobalRefreshButtonProps) {
  ...
  const handleRefresh = useCallback(async () => {
    ...
    // ✅ FIXED: Always use the current dateRange prop to respect selected date range
    let freshDateRange = dateRange;
    
    console.log('[GlobalRefresh] 🔍 Using dateRange prop:', dateRange ? `${dateRange.from.toISOString().split('T')[0]} to ${dateRange.to.toISOString().split('T')[0]}` : 'undefined');
    
    // Only fall back to global variable if prop dateRange is missing
    if (!freshDateRange && typeof window !== 'undefined' && (window as any)._currentDateRange) {
      try {
        ...
        console.log('[GlobalRefresh] 🔍 Fallback to global dateRange:', `${freshDateRange.from.toISOString().split('T')[0]} to ${freshDateRange.to.toISOString().split('T')[0]}`);
      } catch (error) {
        console.log('[GlobalRefresh] ⚠️ Error accessing global dateRange');
      }
    }

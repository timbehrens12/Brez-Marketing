-- Fix outreach database schema
-- This migration fixes the fundamental schema issue where outreach_campaigns 
-- was designed for one-to-one relationships but the code expects many-to-many

-- First, drop existing tables to avoid conflicts (in correct order due to foreign keys)
DROP TABLE IF EXISTS outreach_tasks CASCADE;
DROP TABLE IF EXISTS outreach_messages CASCADE;
DROP TABLE IF EXISTS outreach_campaign_leads CASCADE;
DROP TABLE IF EXISTS outreach_campaigns CASCADE;

-- Create outreach_campaigns table (many campaigns per user, no direct lead_id)

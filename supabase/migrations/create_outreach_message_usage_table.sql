...
CREATE TABLE IF NOT EXISTS outreach_message_usage (
  ...
  estimated_cost DECIMAL(6,4) DEFAULT 0.02, -- Estimated cost in USD
  ai_generated BOOLEAN DEFAULT TRUE,
  
  -- Indexes for efficient queries
  INDEX idx_outreach_usage_user_id (user_id),
  INDEX idx_outreach_usage_generated_at (generated_at),
  INDEX idx_outreach_usage_user_time (user_id, generated_at),
  INDEX idx_outreach_usage_lead_id (lead_id)
);

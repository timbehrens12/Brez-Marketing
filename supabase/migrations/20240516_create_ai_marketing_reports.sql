...
-- Create policies
-- Users can view reports for brands they have access to
CREATE POLICY "Users can view their brands' reports" 
  ...
  USING (
    EXISTS (
      ...
      WHERE user_brands.brand_id = ai_marketing_reports.brand_id 
      AND user_brands.user_id = auth.uid()
    )
  );

-- Users can create reports for brands they have access to
